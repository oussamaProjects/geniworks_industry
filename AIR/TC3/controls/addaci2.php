...
$gwyes = !empty($_POST['gwyes3']);
$cleanyes=!empty($_POST['cleanyes3']);
$eqpic = $_FILES['eqpic3']['name'];
 ...
 if($eqpic!='')
 {
 	...
 	if(move_uploaded_file($_FILES['eqpic3']['tmp_name'], $upload_directory.$pic)){    
		$QueryInsertFile=$cdb->prepare("INSERT INTO asset_clean_inspection VALUES ('','32', '$ast', '$subast', '$pic', '$cleanyes', '$gwyes', NOW(), 'Not Defined')");                   
