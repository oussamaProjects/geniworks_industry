...
$eqpic = $_FILES['skipID']['name'];
 ...
 if($eqpic!='')
 {
 	...
 	if(move_uploaded_file($_FILES['skipID']['tmp_name'], $upload_directory.$pic)){    
		$QueryInsertFile=$cdb->prepare("INSERT INTO asset_clean_inspection VALUES ('','11', '$ast', '$subast', '$subastid1', '$pic', '$cleanyes', '$gwyes', NOW(), '$userpu')");
...
$rees=$QueryInsertFile->get_result();
			...
			echo '<img src="'.$upload_directory.''.$pic.'" class="img-thumbnail" width="300" height="250" />';?>

			<h4><span > <?php echo $ast;?></span> | <span> <?php echo $subast;?> </span> | <span> <?php echo $subastid1;?></span></h4>
