...
$showpers=$cdb->prepare("SELECT * FROM report WHERE rprt_test_type='MT' ORDER BY rprt_asset_id ASC");
$showpers->execute();
if ($showpers===false) {
  ...
  $resrep2=$showpers->get_result();
  	?>
		<div class="tab-pane fade" id="report_type2" role="tabpanel">
													<div class="kt-portlet">
														<div class="kt-portlet__head">
															<div class="kt-portlet__head-label">
																<h3 class="kt-portlet__head-title">Magnetic Testing 
<?php
require('controls/zdze8efzedzdeezd/de5.php');
...
if ($showpers2===false) {
  ...
  $resrep21=$showpers2->get_result();
?>
																	<small><?php while ($showrep21=$resrep21->fetch_row()) { ?><?php echo $showrep21[0];}?> Reports</small>
<?php
}
?>
																</h3>
										...
										<div class="kt-portlet__body">
											...
											<?php while ($showrep2=$resrep2->fetch_row()) { ?>
											<div class="accordion accordion-light" id="accordionExample2">
												<div class="card">
													<div class="card-header" id="headingOne2"><footer class="blockquote-footer" style="float: right; padding-right: 3px;">On <cite title="Source Title"><?php echo $showrep2[43];?></cite></footer>
														<div class="card-title" data-toggle="collapse" style="padding: 0px;" data-target="#collapseOne<?php echo $showrep2[0];?>" aria-expanded="true" aria-controls="collapseOne<?php echo $showrep2[0];?>">
															<?php echo $showrep2[2].$showrep2[3];?> | <?php echo 'R' .$showrep2[1].'-EGT-'.$showrep2[3].'-00'.$showrep2[0].'';?>
														</div>
													</div>
													<div id="collapseOne<?php echo $showrep2[0];?>" class="collapse" aria-labelledby="headingOne2" data-parent="#accordionExample2">
														<div class="card-body">
																	...
																	<div class="kt-section__body">
																		...
																		<div class="form-group row"  style="">
																			...
																			<label class="form-control-label">Asset</label>
																				...
																				<input class="form-control" name="upname" type="text" value="<?php echo $showrep2[2];?>">
																			</div>
																			<div class="col-lg-4 col-xl-4">
																			<label class="form-control-label">AssetID</label>
																			
																			<input class="form-control" name="upname" type="text" value="<?php echo $showrep2[3];?>">
																			</div>
																			<div class="col-lg-4 col-xl-4">
...
?>
                                                                <select name="subassetchos" class="form-control kt_selectpicker">
                                                                    <option value="<?php echo $showrep2[4];?>" selected="selected"><?php echo $showrep2[4];?></option>
                                                                    <?php
...
?>
																		...
																		<div class="form-group row">
																			...
																			<label class="form-control-label">Equipment</label>
																				...
																				<input class="form-control" type="text" readonly="readonly" value="<?php echo $showrep2[27];?>">
																			...
																			<div class="col-lg-4 col-xl-4">
																				...
																				<div class="input-group">
																					<input type="text" class="form-control" name="upphone" value="<?php echo $showrep2[28];?>" placeholder="Phone" aria-describedby="basic-addon1">
																				</div>
																				<span class="form-text text-muted"></span>
																			</div>
...
<?php
require('controls/zdze8efzedzdeezd/de5.php');
$selhma=$cdb->prepare("SELECT rprt_marka,rprt_verdicta FROM report WHERE rprt_id='$showrep2[0]'");
$selhma->execute();
if ($selhma===false) {
  echo "";
}
...
<?php
require('controls/zdze8efzedzdeezd/de5.php');
$selhmb=$cdb->prepare("SELECT rprt_markb,rprt_verdictb FROM report WHERE rprt_id='$showrep2[0]'");
$selhmb->execute();
if ($selhmb===false) {
  echo "";
}
...
<?php
require('controls/zdze8efzedzdeezd/de5.php');
$selhmc=$cdb->prepare("SELECT rprt_markc,rprt_verdictc FROM report WHERE rprt_id='$showrep2[0]'");
$selhmc->execute();
if ($selhmc===false) {
  echo "";
}
...
<?php
require('controls/zdze8efzedzdeezd/de5.php');
$selhmd=$cdb->prepare("SELECT rprt_markd,rprt_verdictd FROM report WHERE rprt_id='$showrep2[0]'");
$selhmd->execute();
if ($selhmd===false) {
  echo "";
}
...
<?php
require('controls/zdze8efzedzdeezd/de5.php');
$selhme=$cdb->prepare("SELECT rprt_marke,rprt_verdicte FROM report WHERE rprt_id='$showrep2[0]'");
$selhme->execute();
if ($selhme===false) {
  echo "";
}
...
<?php
require('controls/zdze8efzedzdeezd/de5.php');
$selhmf=$cdb->prepare("SELECT rprt_markf,rprt_verdictf FROM report WHERE rprt_id='$showrep2[0]'");
$selhmf->execute();
if ($selhmf===false) {
  echo "";
}
...
<?php
require('controls/zdze8efzedzdeezd/de5.php');
$selhmg=$cdb->prepare("SELECT rprt_markg,rprt_verdictg FROM report WHERE rprt_id='$showrep2[0]'");
$selhmg->execute();
if ($selhmg===false) {
  echo "";
}
...
<?php
require('controls/zdze8efzedzdeezd/de5.php');
$selhmh=$cdb->prepare("SELECT rprt_markh,rprt_verdicth FROM report WHERE rprt_id='$showrep2[0]'");
$selhmh->execute();
if ($selhmh===false) {
  echo "";
}
...
<?php
require('controls/zdze8efzedzdeezd/de5.php');
$selhmi=$cdb->prepare("SELECT rprt_marki,rprt_verdicti FROM report WHERE rprt_id='$showrep2[0]'");
$selhmi->execute();
if ($selhmi===false) {
  echo "";
}
...
<?php
require('controls/zdze8efzedzdeezd/de5.php');
$selhmj=$cdb->prepare("SELECT rprt_markj,rprt_verdictj FROM report WHERE rprt_id='$showrep2[0]'");
$selhmj->execute();
if ($selhmj===false) {
  echo "";
}
...
<?php
require('controls/zdze8efzedzdeezd/de5.php');
$selhmk=$cdb->prepare("SELECT rprt_markk,rprt_verdictk FROM report WHERE rprt_id='$showrep2[0]'");
$selhmk->execute();
if ($selhmk===false) {
  echo "";
}
...
?>																		
																		...
																		<div class="row">
																			<div class="col-lg-3 col-xl-3">
																				...
																				<div class="input-group">
																					<a href="#" data-toggle="modal" data-target="#imgmodal<?php echo $showrep2[0];?>"><img name="imgup" src="controls/<?php echo $showrep2[48];?>" id="myImg<?php echo $showrep2[0];?>" alt="<?php echo $showrep2[3];?>" style="height:70px;"></a>
																				</div>
																				<span class="form-text text-muted"></span>
																			</div>
																			<div class="col-lg-3 col-xl-3">
																				...
																				<div class="input-group">
																					<a href="#" data-toggle="modal" data-target="#imgmodal1<?php echo $showrep2[0];?>"><img name="imgup" src="controls/<?php echo $showrep2[52];?>" id="myImg<?php echo $showrep2[0];?>" alt="<?php echo $showrep2[3];?>" style="height:70px;"></a>
																				</div>
																				<span class="form-text text-muted"></span>
																			</div>
																			<div class="col-lg-3 col-xl-3">
																				...
																				<div class="input-group">
																					<a href="#" data-toggle="modal" data-target="#imgmodal2<?php echo $showrep2[0];?>"><img name="imgup" src="controls/<?php echo $showrep2[53];?>" id="myImg<?php echo $showrep2[0];?>" alt="<?php echo $showrep2[3];?>" style="height:70px;"></a>
																				</div>
																				<span class="form-text text-muted"></span>
																			</div>
																			<div class="col-lg-3 col-xl-3">
																				...
																				<div class="input-group">
																					<a href="#" data-toggle="modal" data-target="#imgmodal3<?php echo $showrep2[0];?>"><img name="imgup" src="controls/<?php echo $showrep2[54];?>" id="myImg<?php echo $showrep2[0];?>" alt="<?php echo $showrep2[3];?>" style="height:70px;"></a>
																				</div>
																				<span class="form-text text-muted"></span>
																			</div>
<?php
require('controls/zdze8efzedzdeezd/de5.php');
$selectpic=$cdb->prepare("SELECT * FROM report_pic WHERE rertp_report_id='$showrep2[0]'");
$selectpic->execute();
if ($selectpic===false) {
  echo "We Have Problem In Data Export!";
}
...
?>																		</div>
												...
												<div class="col-lg-4 col-xl-4">
													...
													<input class="form-control" type="text" name="uvtypeup"  value="<?php echo $showrep2[29];?>">	
												</div>
													<div class="col-lg-4 col-xl-4">
													<label class="form-control-label">UV current Value</label>
												
													<input class="form-control" type="text" name="uvvalup"  value="<?php echo $showrep2[31];?>">
												</div>
												<div class="col-lg-4 col-xl-4">
													<label class="form-control-label">Witness Type *</label>
												
													<input class="form-control" type="text"  name="witnsup"  value="<?php echo $showrep2[32];?>">
												</div>
												<div class="col-lg-4 col-xl-4">
													<label class="form-control-label">MFV *</label>
												
													<input class="form-control" type="text"  name="mfvup" value="<?php echo $showrep2[33];?>">
													</div>
													<div class="col-lg-4 col-xl-4">
													<label class="form-control-label">Prod Space *</label>
												
													<input class="form-control" type="text"  name="prspaceup" value="<?php echo $showrep2[34];?>">
													</div>
													<div class="col-lg-4 col-xl-4">
													<label class="form-control-label">Demagneted *</label>
												
													<input class="form-control" type="text"  name="demagup" value="<?php echo $showrep2[35];?>">
													</div>
													<div class="col-lg-4 col-xl-4">
													<label class="form-control-label">Demagnetising Delay *</label>
												
													<input class="form-control" type="text"  name="demagndeup"  value="<?php echo $showrep2[36];?>">
													</div>
													<div class="col-lg-4 col-xl-4">
												    <label class="form-control-label">Revealer *</label>
												
													<input class="form-control" type="text"  name="revealerup" value="<?php echo $showrep2[37];?>">
												</div>
												<div class="col-lg-4 col-xl-4">
													<label class="form-control-label">Light Used *</label>
												
													<input class="form-control" type="text"  name="lightp"  value="<?php echo $showrep2[38];?>">
												</div>
												<div class="col-lg-4 col-xl-4">
													<label class="form-control-label">Artificial Light Value *</label>
												
													<input class="form-control" type="text"  name="artifiup"  value="<?php echo $showrep2[39];?>">
												</div>
												<div class="col-lg-4 col-xl-4">
													<label class="form-control-label">Brightness Value *</label>
												
													<input class="form-control" type="text"  name="brightup" value="<?php echo $showrep2[40];?>">
													</div>
												</div>
												<div class="row"  style="background-color: #f0f0f0;">
																		...
																		<div class="form-group row"  style="background-color: #f0f0f0;">
																			...
																			<?php include('controls/proccesmt.php');?>
																			<?php include('controls/specificmt.php');?>
																			
																			<?php include('controls/acceptancemt.php');?>
																			<?php // include('controls/result2.php');?>
																			<br>
																			<br><br><br><br>
																...
																<div class="kt-form__actions">
																	<div class="row">
																		<div class="col-lg-9 col-xl-9">
																			...
																			<input type="hidden" name="idrep" value="<?php echo $showrep2[0];?>">
																			<button type="submit" name="saveup" class="btn btn-success">Save</button>
																			<button type="reset" class="btn btn-default">Edit</button>
																			<a href="pdf/output.php?id_r=<?php echo $showrep2[0];?>" target="_Blank" class="btn btn-brand">PDF</a>&nbsp;
																		</div>
																	</div>
																</div>
															</div>
...
<div class="modal fade" id="imgmodal<?php echo $showrep2[0];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<button type="button" class="close" style="font-size:24px; color: white;" data-dismiss="modal" aria-label="Close">
														...
														<div class="modal-dialog" role="document">
															<div class="modal-content">
																...
																<img name="imgup" src="controls/<?php echo $showrep2[48];?>" id="myImg<?php echo $showrep2[0];?>" alt="<?php echo $showrep2[3];?>" style="width:100%;">
															</div>
														</div>
													</div>
<div class="modal fade" id="imgmodal1<?php echo $showrep2[0];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<button type="button" class="close" style="font-size:24px; color: white;" data-dismiss="modal" aria-label="Close">
																		<span aria-hidden="true">&times;</span>
																	</button>
														<div class="modal-dialog" role="document">
															<div class="modal-content">
																...
																<img name="imgup" src="controls/<?php echo $showrep2[52];?>" id="myImg<?php echo $showrep2[0];?>" alt="<?php echo $showrep2[3];?>" style="width:100%;">
															</div>
														</div>
													</div>
<div class="modal fade" id="imgmodal2<?php echo $showrep2[0];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<button type="button" class="close" style="font-size:24px; color: white;" data-dismiss="modal" aria-label="Close">
																		<span aria-hidden="true">&times;</span>
																	</button>
														<div class="modal-dialog" role="document">
															<div class="modal-content">
																...
																<img name="imgup" src="controls/<?php echo $showrep2[53];?>" id="myImg<?php echo $showrep2[0];?>" alt="<?php echo $showrep2[3];?>" style="width:100%;">
															</div>
														</div>
													</div>

<div class="modal fade" id="imgmodal3<?php echo $showrep2[0];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<button type="button" class="close" style="font-size:24px; color: white;" data-dismiss="modal" aria-label="Close">
																		<span aria-hidden="true">&times;</span>
																	</button>
														<div class="modal-dialog" role="document">
															<div class="modal-content">
																...
																<img name="imgup" src="controls/<?php echo $showrep2[54];?>" id="myImg<?php echo $showrep2[0];?>" alt="<?php echo $showrep2[3];?>" style="width:100%;">
															</div>
														</div>
													</div>
    <?php
...
<script type="text/javascript">
	$("#resn2<?php echo $showrep2[0];?>").on("change", function () {        
      $modal = $('#resultmodal2');
      if($(this).val() === 'newres2'){
        $modal.modal('show');
    }
...
<script type="text/javascript">
	$("#accepmn<?php echo $showrep2[0];?>").on("change", function () {        
      $modal = $('#acceptancemodalmt');
      if($(this).val() === 'newaccepm'){
        $modal.modal('show');
    }
 });
</script>
<script type="text/javascript">
	$("#specmtn<?php echo $showrep2[0];?>").on("change", function () {        
      $modal = $('#specificmodalmt');
      if($(this).val() === 'newspecmt'){
        $modal.modal('show');
    }
 });
</script>
<script type="text/javascript">
	$("#prosmtn<?php echo $showrep2[0];?>").on("change", function () {        
      $modal = $('#proccesmodalmt');
      if($(this).val() === 'newprosmt'){
        $modal.modal('show');
    }
 });
</script>
...
<?php include('controls/newresl2.php');?>
<?php include('controls/newaccmt.php');?>
<?php include('controls/newspecificmt.php');?>
<?php include('controls/newproccesmt.php');?>