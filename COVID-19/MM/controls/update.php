...
$addlog=$cdb->prepare("UPDATE cabins SET cabn_status='$updatedstat', cabn_last_ops=NOW(), cabn_last_op_team='$upuserid' WHERE cabn_name='$upcabinid' AND cabn_owner LIKE 'MARSA-MAROC DE%'");
	     $addlog->execute();
	     if ($addlog === false) {
       ...
       $addconlog=$cdb->prepare("INSERT INTO cabin_cleaning_history VALUES ('','$upcabinid', '$upuserid', '$updatedstat', '', NOW(), '', 'MARSA-MAROC DE', NOW())"); 
         $addconlog->execute();
       if ($addconlog === false) {
          echo "display error".mysqli_error($cdb);
       }
